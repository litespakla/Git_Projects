'''
Each of the six faces on a cube has a different digit (0 t0 9) written on it; 
...
for i in range(len(subsets)-1):
    for j in range(i, len(subsets)):
        if '6' in subsets[i] and '9' not in subsets[i]:
            subsets[i]=subsets[i]+('9',)
        if '9' in subsets[i] and '6' not in subsets[i]:
            subsets[i]=subsets[i]+('6',)
        if '6' in subsets[j] and '9' not in subsets[j]:
            subsets[j]=subsets[j]+('9',)
        if '9' in subsets[j] and '6' not in subsets[j]:
            subsets[j]=subsets[j]+('6',)

        if all((k[0] in subsets[i] and k[1] in subsets[j]) or (k[0] in subsets[j] and k[1] in subsets[i]) for k in squares):
            count+=1
